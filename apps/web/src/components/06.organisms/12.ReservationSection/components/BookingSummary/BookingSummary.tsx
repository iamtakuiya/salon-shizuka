...
/** Booking summary + contact form (right column on desktop) */
export default function BookingSummary({
  ...
  selectedAddons: (typeof import('@/utils/constants').ADDONS)[number][];
  total: number;
      ...
      <div className={styles.summary}>
        ...
        {lineItems.length === 0 ? (
          ...
          <>
            ...
            <p className={styles.summary__note}>表記価格は税込・基本料金です。髪の状態により変動する場合があります。</p>
          </>
        )}
      </div>
      ...
      <form onSubmit={onSubmit} className={styles.contactForm} noValidate>
        ...
        <div className={styles.contactForm__field}>
          ...
          <input
            ...
            className={[styles.contactForm__input, errors.name ? styles['contactForm__input--error'] : ''].join(' ')}
            {...register('name')}
          />
          {errors.name?.message && <p className={styles.contactForm__error} role="alert">{errors.name.message}</p>}
        </div>
        <div className={styles.contactForm__field}>
          ...
          <input
            ...
            className={[styles.contactForm__input, errors.email ? styles['contactForm__input--error'] : ''].join(' ')}
            {...register('email')}
          />
        ...
        <button type="submit" className={styles.btn}>内容を確認する</button>
      </form>
    </div>
  );
}
